...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.КомуКопии) = Тип("Массив") Тогда
	...
	//Блок копии
	Если ТипЗнч(Параметры.КомуКопии) = Тип("СписокЗначений") Тогда
		ПочтовыйАдресПолучателяКопии = "";
		Для Каждого ЭлементПочтовыйАдрес Из Параметры.КомуКопии Цикл
			Если ЗначениеЗаполнено(ЭлементПочтовыйАдрес.Представление) Тогда 
				ПочтовыйАдресПолучателяКопии = ПочтовыйАдресПолучателя
										+ ЭлементПочтовыйАдрес.Представление
										+ " <"
										+ ЭлементПочтовыйАдрес.Значение
										+ ">; "
	...
	ИначеЕсли ТипЗнч(Параметры.КомуКопии) = Тип("Строка") Тогда
		ПочтовыйАдресПолучателяКопии = Параметры.КомуКопии;
	ИначеЕсли ТипЗнч(Параметры.КомуКопии) = Тип("Массив") Тогда
		Если Параметры.КомуКопии.Количество() > 1 Тогда
			Элементы.ПочтовыйАдресПолучателяКопии.КнопкаВыбора = Истина;
		КонецЕсли;
		Для Каждого СтруктураПолучателя Из Параметры.КомуКопии Цикл
			ЕстьСвойствоВыбран = СтруктураПолучателя.Свойство("Выбран");
			МассивАдресов = СтрРазделить(СтруктураПолучателя.Адрес, ";");
			Для Каждого Адрес Из МассивАдресов Цикл
				Если ПустаяСтрока(Адрес) Тогда 
				...
				КонецЕсли;
				Если (ЕстьСвойствоВыбран И СтруктураПолучателя.Выбран) ИЛИ (НЕ ЕстьСвойствоВыбран) Тогда
					ПочтовыйАдресПолучателяКопии = ПочтовыйАдресПолучателяКопии + СтруктураПолучателя.Представление + " <" + СокрЛП(Адрес) + ">; ";
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
	...
	Если ТипЗнч(Параметры.КомуКопии) = Тип("Массив") Тогда
		ПолучателиСообщенияКопии = ПоместитьВоВременноеХранилище(Параметры.КомуКопии, УникальныйИдентификатор);
	Иначе
		ПолучателиСообщенияКопии = ПоместитьВоВременноеХранилище(Новый Массив, УникальныйИдентификатор);
	КонецЕсли;
	//
...
&НаКлиенте
Процедура ПочтовыйАдресПолучателяКопииНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ПолучателиСообщения", ПолучателиСообщенияКопии);
	ПараметрыФормы.Вставить("ПочтовыйАдресПолучателя", Элемент.ТекстРедактирования);
	ОткрытьФорму("ОбщаяФорма.ВыборПолучателейСообщения", ПараметрыФормы, Элемент);
	
КонецПроцедуры

&НаКлиенте
Процедура ПочтовыйАдресПолучателяКопииОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		ПочтовыйАдресПолучателяКопии = ВыбранноеЗначение.ПочтовыйАдресПолучателя;
	КонецЕсли;
...
Функция ОтправитьПочтовоеСообщение()
	...
	//+Егор, 09.01.2017, вставим текст файла в тело письма
	ТекстХТМЛ = "";
	Для Каждого Вложение Из Вложения Цикл
		Если Вложение.Представление = "Оценка выполнения работ.html" ИЛИ Вложение.Представление = "Заказ-наряд.html" Тогда
			ИмяВременнойПапки = ПолучитьИмяВременногоФайла();
			СоздатьКаталог(ИмяВременнойПапки);
				
			ИмяВременногоФайла = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ИмяВременнойПапки) + Вложение.Представление;
	...
	Если ТекстХТМЛ <> "" Тогда
		ОсновнойТекстНачало = СтрНайти(ПараметрыПисьма.Тело, "<body>")+7;
		ОсновнойТекстКонец = СтрНайти(ПараметрыПисьма.Тело, "</body>", НаправлениеПоиска.СКонца);
		ОсновнойТекстПисьма = Сред(ПараметрыПисьма.Тело, ОсновнойТекстНачало, ОсновнойТекстКонец-ОсновнойТекстНачало);
		
		ДопТекстНачало = СтрНайти(ТекстХТМЛ, "<TABLE",НаправлениеПоиска.СНачала,,1)-1;
		НовыйТекстПисьма = Лев(ТекстХТМЛ, ДопТекстНачало) + ОсновнойТекстПисьма + Прав(ТекстХТМЛ, СтрДлина(ТекстХТМЛ)-ДопТекстНачало);
		
		ПараметрыПисьма.Вставить("Тело", НовыйТекстПисьма);
	КонецЕсли;
	//-
...
Функция СформироватьПараметрыПисьма()
	...
	//Блок копии
	СписокПолучателей = ОбщегоНазначенияКлиентСервер.АдресаЭлектроннойПочтыИзСтроки(ПочтовыйАдресПолучателяКопии);
	Копии = Новый Массив;
	Для Каждого Получатель Из СписокПолучателей Цикл
		Если Не ПустаяСтрока(Получатель.ОписаниеОшибки) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				Получатель.ОписаниеОшибки, , "ПочтовыйАдресПолучателяКопии");
			Возврат Неопределено;
		КонецЕсли;
		Копии.Добавить(Новый Структура("Адрес, Представление", Получатель.Адрес, Получатель.Псевдоним));
	КонецЦикла;
...
&НаСервере
Процедура ВывестиФайлНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ВывестиФайл(Команда)
	ВывестиФайлНаСервере();
КонецПроцедуры
