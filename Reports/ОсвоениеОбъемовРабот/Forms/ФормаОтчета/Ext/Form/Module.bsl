...
Процедура ПриОбновленииСоставаПользовательскихНастроекНаСервере(СтандартнаяОбработка)
	...
	//+Егор, 06.02.2017, установка настроек для открытия из начальной страницы 
	Если Параметры.Свойство("ПользовательскиеНастройки") И Параметры.ПользовательскиеНастройки<>Неопределено Тогда
		Период = Параметры.ПользовательскиеНастройки.ДополнительныеСвойства.Период;
		Настройки = Отчет.КомпоновщикНастроек.ПолучитьНастройки();
		РасширениеПечати.УстановитьПараметр(Настройки, "Период", Период, Истина);
		Отчет.КомпоновщикНастроек.ЗагрузитьНастройки(Настройки);
		
		ЗначениеПараметра = РасширениеПечати.ПолучитьПараметр(Настройки, "Период");
		ЭтаФорма["ЗначениеПараметраНастроек_Значение_"+СтрЗаменить(ЗначениеПараметра.ИдентификаторПользовательскойНастройки,"-","")] = Период;
	КонецЕсли;
