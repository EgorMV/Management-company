...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.ВыбранныеРаботы.Видимость = Ложь;
КонецПроцедуры
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	мРезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка);
КонецПроцедуры
...
#Область ОбработкаВыбранныхРабот

&НаКлиенте
Процедура мРезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
...
//Здесь заполняем колонки, на которые должна реагировать расшифровка. В данном варианте отчета поля д.б. ссылками 
    ...
    МРасшифровываемых.Добавить("Работа");
...
// Прописываем доступные действия. Если какое то действие не указать, оно не будет доступно пользователю
    ...
    ДопПункты.Добавить("ОценитьРаботу", "Оценить работу");
	ДопПункты.Добавить("ДобавитьРаботуДляОценки", "Добавить работу для оценки");

//Меню 
    ОповещениеОВыборе = Новый ОписаниеОповещения("мРезультатОбработкаРасшифровки_Продолжение", ЭтаФорма, Расшифровка);
    ОбработкаРасшифровки.ПоказатьВыборДействия(ОповещениеОВыборе, Расшифровка, , ДопПункты);

КонецПроцедуры

&НаКлиенте
Процедура мРезультатОбработкаРасшифровки_Продолжение(ВыполненноеДействие, ПараметрВыполненногоДействия, ДополнительныеПараметры) Экспорт
	
	Если ВыполненноеДействие = "ОценитьРаботу" Тогда
		ОценитьРаботу(ДополнительныеПараметры);
	ИначеЕсли ВыполненноеДействие = "ДобавитьРаботуДляОценки" Тогда
		ДобавитьРаботуДляОценки(ДополнительныеПараметры);	
	Иначе
...
&НаКлиенте
Процедура ОценитьРаботу(Расшифровка)
	
	ВыбРабота = ПолучитьЗначениеРасшифровкиНаСервере(Расшифровка);
	МассивРабот = Новый Массив;
	МассивРабот.Добавить(ВыбРабота.Работа);
	
	Форма = ПолучитьФорму("Документ.ОценкаКачестваРабот.Форма.ФормаДокумента");
    ДанныеФормы = Форма.Объект;
    ОценитьВыбранныеРаботыНаСервере(ДанныеФормы, МассивРабот);
...
&НаКлиенте
Процедура ДобавитьРаботуДляОценки(Расшифровка)
	
	ВыбРабота = ПолучитьЗначениеРасшифровкиНаСервере(Расшифровка);
	
	Если ВыбранныеРаботы.НайтиСтроки(Новый Структура("Работа", ВыбРабота.Работа)).Количество() > 0 Тогда
		Возврат;
	КонецЕсли;
	
	НСтрока = ВыбранныеРаботы.Добавить();
	НСтрока.Работа = ВыбРабота.Работа;
	НСтрока.Проект = ВыбРабота.Проект;
	
	Если Не Элементы.ВыбранныеРаботы.Видимость Тогда
		ПоказатьВыбранныеРаботы(Команды.ПоказатьВыбранныеРаботы);
	КонецЕсли;
	
КонецПроцедуры	

&НаСервере
Функция ПолучитьЗначениеРасшифровкиНаСервере(Расшифровка)
	
	Данные = ПолучитьИзВременногоХранилища(ОтчетДанныеРасшифровки);
    ИскРабота = Данные.Элементы.Получить(Расшифровка).ПолучитьПоля()[0].Значение;	
    Возврат Новый Структура("Работа, Проект", ИскРабота, ИскРабота.Владелец);
...
Функция ПолучитьРасшифровкуНаСервере(Расшифровка, МассивИменРасшифровки, ЭтоГруппа = Ложь)
    ...
    Для Каждого ИмяРасшифровки Из МассивИменРасшифровки Цикл
        ИскПоле = Поля.Найти(ИмяРасшифровки);
		Если ИскПоле <> Неопределено Тогда 
			ИскЗначение = ИскПоле.Значение; 
		КонецЕсли;
	КонецЦикла;
...
&НаСервереБезКонтекста
Процедура ОценитьВыбранныеРаботыНаСервере(НДок, Знач ВыбранныеРаботы)

	Если ТипЗнч(ВыбранныеРаботы) = Тип("ДанныеФормыКоллекция") Тогда
		ВыбранныеРаботы = ВыбранныеРаботы.Выгрузить().ВыгрузитьКолонку("Работа");
	КонецЕсли;	
	
	НДок.Дата = ТекущаяДата();
	НДок.Проект = ВыбранныеРаботы[0].Владелец;
	Для Каждого Стр Из ВыбранныеРаботы Цикл
		НСтрока = НДок.Работы.Добавить();
		НСтрока.Работа = Стр;
	КонецЦикла;
...
&НаКлиенте
Процедура ОценитьВыбранныеРаботы(Команда)
	
	Отказ = Ложь;
	Если ВыбранныеРаботы.Количество() = 0 Тогда
		Отказ = Истина; 	
	Иначе
		ТекПроект = ВыбранныеРаботы[0].Работа;
		Для Каждого Стр Из ВыбранныеРаботы Цикл
			Если ТекПроект<>Стр.Работа Тогда
				Сообщить("Нельзя оценивать работы по разным проектам!", СтатусСообщения.Важное);
				Отказ = Истина;
				Прервать;
			КонецЕсли;			
		КонецЦикла;		
	...
	Форма = ПолучитьФорму("Документ.ОценкаКачестваРабот.Форма.ФормаДокумента");
    ДанныеФормы = Форма.Объект;
    ОценитьВыбранныеРаботыНаСервере(ДанныеФормы, ВыбранныеРаботы);
    КопироватьДанныеФормы(ДанныеФормы, Форма.Объект);
    Форма.Открыть();
...
&НаКлиенте
Процедура ПоказатьВыбранныеРаботы(Команда)
	
	ПоказатьВыбРаботы = Элементы.ПоказатьВыбранныеРаботы.Пометка;
	Элементы.ПоказатьВыбранныеРаботы.Пометка = НЕ ПоказатьВыбРаботы;
	Элементы.ВыбранныеРаботы.Видимость = НЕ ПоказатьВыбРаботы;
