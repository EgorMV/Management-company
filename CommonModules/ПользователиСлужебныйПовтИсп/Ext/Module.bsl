...
// Содержит сохраненные параметры, используемые подсистемой.
Функция АдминистраторЗапретилОткрыватьВнешниеОтчетыИОбработки() Экспорт
	Если ПустаяСтрока(ПользователиИнформационнойБазы.ТекущийПользователь().Имя) Тогда
		Возврат Ложь; // В базе без пользователей запрет не требуется.
	КонецЕсли;
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		Возврат Ложь; // В модели сервиса запрет не требуется.
	КонецЕсли;
	...
	Если Не ПараметрыАдминистрирования.Свойство("РазрешеноОткрытиеВнешнихОтчетовИОбработок") Тогда
		Возврат Ложь; // Решение еще не принято.
	Иначе
		Возврат Не ПараметрыАдминистрирования.РазрешеноОткрытиеВнешнихОтчетовИОбработок;
	КонецЕсли;
КонецФункции
