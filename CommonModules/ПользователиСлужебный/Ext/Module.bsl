...
// Вызывается при переходе на новую версию конфигурации 2.3.3.59.
Процедура ДобавитьАдминистраторамПравоОткрытияВнешнихОтчетовИОбработок() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
		...
		МодульУправлениеДоступомСлужебный.ДобавитьВПрофильАдминистратораРольИнтерактивноеОткрытиеВнешнихОтчетовИОбработок();
	КонецЕсли;
...
Функция КлючПредупрежденияБезопасностиПриЗапуске()
	...
	РазрешеноОткрытиеВнешнихОтчетовИОбработок = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыАдминистрирования, "РазрешеноОткрытиеВнешнихОтчетовИОбработок");
	УстановитьПривилегированныйРежим(Ложь);
	
	ЭтоАдминистраторСистемы = Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь);
	Если ЭтоАдминистраторСистемы И РазрешеноОткрытиеВнешнихОтчетовИОбработок = Неопределено Тогда
		Возврат "ПослеОбновления";
	КонецЕсли;
	
	Если РазрешеноОткрытиеВнешнихОтчетовИОбработок <> Неопределено Тогда
