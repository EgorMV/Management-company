...
Процедура СоздатьНовоеПисьмоПроверкаУчетнойЗаписиВыполнена(УчетнаяЗаписьНастроена, ПараметрыОтправки) Экспорт
	Перем Отправитель, Получатель, ПолучательКопии, Вложения, Тема, Текст, УдалятьФайлыПослеОтправки;
	...
	ПараметрыОтправки.Свойство("ПолучательКопии", ПолучательКопии);
	ПараметрыОтправки.Свойство("Тема", Тема);
	ПараметрыОтправки.Свойство("Тело", Текст);
	...
	Иначе
		ОткрытьПростуюФормуОтправкиПочтовогоСообщения(Отправитель, Получатель, ПолучательКопии,
...
Процедура ОткрытьПростуюФормуОтправкиПочтовогоСообщения(Отправитель,
			Получатель, ПолучательКопии, Тема,Текст, СписокФайлов, УдалятьФайлыПослеОтправки, ОписаниеОповещенияОЗакрытии)
	
	ПараметрыПисьма = Новый Структура;
	
	ПараметрыПисьма.Вставить("УчетнаяЗапись", Отправитель);
	ПараметрыПисьма.Вставить("Кому", Получатель);
	ПараметрыПисьма.Вставить("КомуКопии", ПолучательКопии);
...
Процедура ОтправитьТабличныеДокументыПроверкаУчетнойЗаписиВыполнена(УчетнаяЗаписьНастроена, Контекст) Экспорт
	...
	//+Егор, 09.01.2017, указание отправляемого формата 
	Если Контекст.ПараметрыОтправки.Свойство("ФорматОтправки") Тогда
		ПараметрыФормы.Вставить("ФорматОтправки", Контекст.ПараметрыОтправки.ФорматОтправки);
	КонецЕсли;
