...
// Удаляет роль ИнтерактивноеОткрытиеВнешнихОтчетовИОбработок из всех профилей групп доступа.
Процедура ОчиститьПрофилиОтРолиИнтерактивноеОткрытиеВнешнихОтчетовИОбработок() Экспорт
	// Роль удаляется из всех профилей, за исключением специально выделенного профиля с одной этой ролью.
	Роль = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.Роли.ИнтерактивноеОткрытиеВнешнихОтчетовИОбработок);
	ИсключаемыйПрофиль = ПрофильОткрытиеВнешнихОтчетовИОбработок(Ложь);
	...
	Запрос.УстановитьПараметр("ИсключаемыйПрофиль", ИсключаемыйПрофиль);
	...
	Для Каждого Профиль Из МассивПрофилей Цикл
		...
		Найденные = ПрофильОбъект.Роли.НайтиСтроки(Новый Структура("Роль", Роль));
...
// Добавляет роль ИнтерактивноеОткрытиеВнешнихОтчетовИОбработок в профиль администратора системы.
Процедура ДобавитьВПрофильАдминистратораРольИнтерактивноеОткрытиеВнешнихОтчетовИОбработок() Экспорт
...
Процедура ОбновитьРолиПользователей(Знач Пользователи1 = Неопределено,
	...
	Если НЕ ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		КонецЕсли;
		Если Не ПользователиСлужебныйПовтИсп.АдминистраторЗапретилОткрыватьВнешниеОтчетыИОбработки() Тогда
			РолиАдминистратора.Вставить("ИнтерактивноеОткрытиеВнешнихОтчетовИОбработок", Истина);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.3.3.65";
	Обработчик.Процедура = "УправлениеДоступомСлужебный.СоздатьПрофильДляОткрытияВнешнихОтчетовИОбработок";
	Обработчик.РежимВыполнения = "Оперативно";
...
// Создает профиль с единственной ролью "ИнтерактивноеОткрытиеВнешнихОтчетовИОбработок" в случае его отсутствия.
Процедура СоздатьПрофильДляОткрытияВнешнихОтчетовИОбработок() Экспорт
	ПрофильОткрытиеВнешнихОтчетовИОбработок(Истина);
КонецПроцедуры
...
// Находит профиль с единственной ролью "ИнтерактивноеОткрытиеВнешнихОтчетовИОбработок" и возвращает его ссылку.
//  Создает профиль в случае отсутствия.
//
// Возвращаемое значение:
//   СправочникСсылка.ПрофилиГруппДоступа - Найденный профиль.
//
Функция ПрофильОткрытиеВнешнихОтчетовИОбработок(СоздатьВСлучаеОтсутствия)
	Роль = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.Роли.ИнтерактивноеОткрытиеВнешнихОтчетовИОбработок);
	
	ИдентификаторПредопределенного = Новый УникальныйИдентификатор("1b3472f6-4d87-11e6-8264-5404a6a6895d");
	СсылкаПредопределенного = Справочники.ПрофилиГруппДоступа.ПолучитьСсылку(ИдентификаторПредопределенного);
	ПрофильСуществует = (ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаПредопределенного, "Ссылка") <> Неопределено);
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПрофилиСРолью.Ссылка КАК Ссылка
	|ИЗ
	|	ПрофилиСРолью КАК ПрофилиСРолью
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПрофилиГруппДоступа.Роли КАК ВсеРолиЭтогоПрофиля
	|		ПО ПрофилиСРолью.Ссылка = ВсеРолиЭтогоПрофиля.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ПрофилиСРолью.Ссылка
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВсеРолиЭтогоПрофиля.Роль) = 1";
	Запрос.УстановитьПараметр("Роль", Роль);
	...
	Иначе
		Если СоздатьВСлучаеОтсутствия Тогда
			// Регистрация профиля "Интерактивное открытие внешних отчетов и обработок".
			ПрофильОбъект = Справочники.ПрофилиГруппДоступа.СоздатьЭлемент();
			ПрофильОбъект.УстановитьСсылкуНового(СсылкаПредопределенного);
			ПрофильОбъект.Наименование = НСтр("ru = 'Открытие внешних отчетов и обработок'", Метаданные.ОсновнойЯзык.КодЯзыка);
			ПрофильОбъект.Комментарий = НСтр(
				"ru = 'Дополнительно назначается тем пользователям, которым должны быть доступно открытие внешних обработок через меню ""Файл"" - ""Открыть"".
